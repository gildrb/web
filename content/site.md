...
I started with an empty repository and wrote the design system, the build script and the verification harness. Authored source and generated pages both live in the repo.
...
The pages are static HTML, CSS and vanilla JavaScript. Vercel Insights and Speed Insights are loaded separately for analytics; the portfolio remains functional if those scripts are unavailable.
...
Layout values are named in CSS. The main article column is 760px wide on desktop. It sits beside a 240px sidebar with a 48px gap. A 6px token handles compact link stacks; larger separations use 24px, 32px, 48px, or 80px according to the relationship between elements. Case titles are 28/36 on desktop and 24/32 on mobile. Body copy is 16/24, while captions and code labels use 14/20. Inter Variable is self-hosted for text and Ioskeley Mono is used for code.
...
The same script rebuilds every page in memory and compares the result with the committed HTML. A source edit without a rebuild fails verification.
...
The site publishes a Schema.org identity graph, WebFinger records, host metadata, an llms.txt reference, an RSS feed, a humans.txt file, and a sitemap. Its `Content-Signal` header permits search, AI input, and AI training.
