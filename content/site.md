...
Functional CSS and JavaScript are inlined by route. The homepage preloads the self-hosted interface and code fonts, and responsive image sets let the browser choose an appropriate file for the viewport. The HTML uses landmarks, live regions, visible keyboard focus, and reduced-motion handling. Vercel keeps clean URLs, permanent legacy redirects, and route-specific discovery headers.
